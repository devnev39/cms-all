...
public class ItemController {
    ...
    // private final String uploadDir = Paths.get(System.getProperty("user.home"), "cms", "uploads",
    // "items").toString();
    ...
    @PostMapping("")
    public Item createItem(@Valid @ModelAttribute NewItemDTO item, BindingResult result)
            throws IOException {
        ...
        if (item.getFile() != null) {
            String key = "uploads/items/" + UUID.randomUUID().toString() + "-"
                    + item.getFile().getOriginalFilename();

            PutObjectRequest putObjectRequest = PutObjectRequest.builder().bucket(bucketName)
                    .key(key).contentType(item.getFile().getContentType()).build();

            s3Client.putObject(putObjectRequest, software.amazon.awssdk.core.sync.RequestBody
                    .fromInputStream(item.getFile().getInputStream(), item.getFile().getSize()));

            String publicUrl =
                    "https://" + bucketName + ".s3." + "ap-south-1" + ".amazonaws.com/" + key;
        ...
        if (dto.getFile().isPresent()) {
            ...
            if (existingItem.getImagePath() != null) {
                DeleteObjectRequest request = DeleteObjectRequest.builder().bucket(bucketName)
                        .key(existingItem.getImagePath()).build();
                s3Client.deleteObject(request);
            }
            // Upload the new file to S3
            String key = "uploads/items/" + UUID.randomUUID().toString() + "-"
                    + dto.getFile().get().getOriginalFilename();
            PutObjectRequest putObjectRequest = PutObjectRequest.builder().bucket(bucketName)
                    .key(key).contentType(dto.getFile().get().getContentType()).build();
            try {
                s3Client.putObject(putObjectRequest,
                        software.amazon.awssdk.core.sync.RequestBody.fromInputStream(
                                dto.getFile().get().getInputStream(),
                                dto.getFile().get().getSize()));
            } catch (IOException e) {
